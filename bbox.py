...
def draw_boxes(image, boxes, labels, obj_thresh, quiet=True):
    for box in boxes:
        ...
        if label >= 0:
            ...
            #                   [box.xmin-3,         box.ymin-height-50], 
            #                   [box.xmin+width+600, box.ymin-height-50], 
            #                   [box.xmin+width+600, box.ymin]], dtype='int32')  
            region = np.array([[box.xmin-3,        box.ymin], 
            ...
            # ポリゴン領域の塗りつぶし　cv2.fillPoly(画像, pts：ポリゴンの配列, npts:各ポリゴンの頂点数の配列, ncontours:領域を区切る輪郭の個数, 色, 線の種類, 頂点座標の小数点以下の桁のビット数)
            cv2.fillPoly(img=image, pts=[region], color=(255, 255, 255))  # 白
            #cv2.fillPoly(img=image, pts=[region], color=(255, 0, 0))  # 青
            ...
            cv2.putText(img=image, 
                        ...
                        fontScale=0.2e-3 * image.shape[0], 
